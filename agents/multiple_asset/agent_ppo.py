...
def load_and_prepare_data(csv_path: str):
    ...
    # Wide raw dataframe: one row per date
    wide_parts = []
    ...
    # Scale all non-date columns for model observations
...
class EpsilonGreedyMultiAssetPolicy:
    ...
    def get_action_probs(self, obs):
        ...
        # dist.distribution is typically a list/tuple of categorical distributions
...
def build_eval_row(base_row, action, probs, positions, net_worth, reward, balance):
    row = dict(base_row)
...
def evaluate_model(model, env, epsilon, save_path=None):
    ...
    while not (terminated or truncated):
        ...
        # uniform fallback when random epsilon action is used
        if probs is None:
...
def main():
    ...
    for split_idx, (raw_train, scaled_train, raw_test, scaled_test) in enumerate(splits, start=1):
        ...
        # ----------------------------------------------------
        # 1) Evaluate on this split's TEST window only
        # ----------------------------------------------------
        ...
        split_eval_path = f"evaluation_split_{split_idx}_multi_asset.csv"
        ...
        # ----------------------------------------------------
        # 2) Evaluate same split model on FULL dataset
        # ----------------------------------------------------
        ...
        full_eval_path = f"evaluation_full_dataset_split_{split_idx}_multi_asset.csv"

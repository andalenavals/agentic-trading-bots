...
PPO outputs under `data/agent_outputs/` are generated by training modules. The repo includes a committed demo snapshot so the trading bots gym can render immediately, but those files can be regenerated.
...
`npm run train:single` trains one PPO agent per CSV under `data/training/commodity_outputs/`, so the single-asset gym can switch between aluminium, copper, and nickel.
